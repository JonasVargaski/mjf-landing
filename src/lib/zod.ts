    ...
    case z.ZodIssueCode.invalid_string:
      if (typeof issue.validation === 'object') {
        if ('startsWith' in issue.validation) {
          ...
          z.util.assertNever(issue.validation as never)
        }
      } else if (issue.validation === 'email') {
        message = `O e-mail precisar ser válido`
      ...
      if (issue.type === 'array' || issue.type === 'set')
        ...
        message = `Deve ser ${
          issue.exact
            ...
            ? `maior que ou igual a `
            : `maior que `
        }${String(new Date(issue.minimum.toString()))}`
      break
      ...
      if (issue.type === 'array' || issue.type === 'set')
        ...
        message = `Deve ser ${
          issue.exact
            ...
            : `smaller than`
        } ${String(new Date(issue.maximum.toString()))}`
      break
